...
class Snowflake:
    ...
    def test_connection(self, __secrets: dict):
        self.logger.log.debug("Testing snowflake connection")
        try:
            con = self.return_connection(__secrets)
            data = {
                "data": {},
    ...
    def run_query(self, query: str, command: bool = False) -> DataFrame:
        self.logger.log.debug("Snowflake - running query")
    ...
    def write_pandas_to_sf(self, df_, table_name):
        ...
        try:
            pdtools.write_pandas(table_name=table_name, conn=con_, df=df_)
